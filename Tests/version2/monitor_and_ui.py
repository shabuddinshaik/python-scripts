...
def save_and_start_monitoring():
    global twilio_client, mail_check_enabled, mail_labels
...
tk.Label(app, text="Mail Body Labels (key:value pairs, comma-separated)").pack()
...
tk.Button(app, text="End Monitoring", command=stop_monitoring).pack()

tk.Label(app, text="Silence Start Time (YYYY-MM-DD HH:MM:SS)").pack()
silence_start_entry = tk.Entry(app)
silence_start_entry.pack()

tk.Label(app, text="Silence End Time (YYYY-MM-DD HH:MM:SS)").pack()
...
status_label = tk.Label(app, text="Status: Not Started")
status_label.pack()
