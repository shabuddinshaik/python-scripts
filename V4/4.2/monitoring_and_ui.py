...
# Helper functions
def load_data():
    global twilio_accounts, urls_to_monitor, alerts, intranet_urls_to_monitor, proxy_settings, regex_patterns
    try:
        with open('monitor_config.json', 'r') as f:
            config = json.load(f)
            twilio_accounts = config.get('twilio_accounts', {})
            urls_to_monitor = config.get('urls_to_monitor', [])
            intranet_urls_to_monitor = config.get('intranet_urls_to_monitor', [])
...
def monitor_urls():
    while monitoring_active:
        for alert in alerts:
            ...
            url = alert['url']
            regex = regex_patterns.get(url)
            status_code, status = check_url(url, regex) if alert['url_type'] == 'internet' else (None, check_intranet_url(url))
            if status:
...
def add_monitoring_url():
    url_type = url_type_var.get()
    url = url_entry.get()
    regex = regex_entry.get()
    proxy = proxy_entry.get()

    if url_type == 'internet':
        urls_to_monitor.append((url, regex))
    else:
        intranet_urls_to_monitor.append(url)

    proxy_settings[url] = proxy

    save_data()
    messagebox.showinfo("Success", "URL added to monitoring list.")

def add_alert():
    alert_name = alert_name_entry.get()
    url = alert_url_var.get()
...
# Monitoring Tab
tk.Label(monitoring_tab, text="URL Type:").grid(row=0, column=0, padx=10, pady=10)
...
url_type_option_menu.grid(row=0, column=1, padx=10, pady=10)

tk.Label(monitoring_tab, text="URL:").grid(row=1, column=0, padx=10, pady=10)
url_entry = tk.Entry(monitoring_tab, width=100)
url_entry.grid(row=1, column=1, padx=10, pady=10)

tk.Label(monitoring_tab, text="Regex to Match (Optional):").grid(row=2, column=0, padx=10, pady=10)
regex_entry = tk.Entry(monitoring_tab, width=100)
regex_entry.grid(row=2, column=1, padx=10, pady=10)

tk.Label(monitoring_tab, text="Proxy URL (Optional):").grid(row=3, column=0, padx=10, pady=10)
proxy_entry = tk.Entry(monitoring_tab, width=100)
proxy_entry.grid(row=3, column=1, padx=10, pady=10)

tk.Button(monitoring_tab, text="Add Monitoring URL", command=add_monitoring_url).grid(row=4, column=1, padx=10, pady=10)
...
tk.Label(alerts_tab, text="URL:").grid(row=1, column=0, padx=10, pady=10)
alert_url_var = tk.StringVar()
alert_url_option_menu = ttk.Combobox(alerts_tab, textvariable=alert_url_var, values=[url for url, _ in urls_to_monitor])
...
alert_twilio_account_option_menu = ttk.Combobox(alerts_tab, textvariable=alert_twilio_account_var, values=list(twilio_accounts.keys()))
