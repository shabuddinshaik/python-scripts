...
def monitor_url(alert):
    while monitoring_active:
        ...
        time.sleep(alert['scrape_interval'])

def start_monitoring(alert_name):
    alert = next((a for a in alerts if a['alert_name'] == alert_name), None)
    if alert and alert_name not in alert_threads:
        thread = Thread(target=monitor_url, args=(alert,))
        thread.daemon = True
        thread.start()
        alert_threads[alert_name] = thread
        logger.info(f"Started monitoring alert: {alert_name}")

def stop_monitoring(alert_name):
    if alert_name in alert_threads:
        del alert_threads[alert_name]
        logger.info(f"Stopped monitoring alert: {alert_name}")
...
def add_alert():
    ...
    alerts.append({
        ...
        'alert_type': 'sms'  # Default to SMS, can be extended to choose call or email
    })
...
def update_alert_list():
    ...
    for alert in alerts:
        ...
        tk.Button(alert_frame, text="Pause", command=lambda a=alert['alert_name']: stop_monitoring(a)).pack(side='left', padx=5)
...
url_entry = tk.Entry(monitoring_tab, width=50)
...
regex_entry = tk.Entry(monitoring_tab, width=50)
...
proxy_entry = tk.Entry(monitoring_tab, width=50)
...
alert_response_codes_entry = tk.Entry(alerts_tab, width=50)
...
silence_reason_entry = tk.Entry(silence_tab, width=50)
