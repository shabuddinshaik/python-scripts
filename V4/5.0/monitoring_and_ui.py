    ...
    try:
        ...
        for message in messages:
            if message.Categories:
                logging.info(f"Message with labels: {message.Subject} - {message.Categories}")
    except Exception as e:
        logging.error(f"Error checking email labels: {e}")

# GUI Setup
root = tk.Tk()
root.title("Alert and Monitoring Application")
...
auth_token_entry = tk.Entry(accounts_tab, show="*")
