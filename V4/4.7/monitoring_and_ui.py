...
tk.Label(alerts_tab, text="Alert Method:").grid(row=5, column=0, padx=10, pady=10)
alert_method_var = tk.StringVar(value="call")
ttk.Combobox(alerts_tab, textvariable=alert_method_var, values=["call", "sms", "email"]).grid(row=5, column=1, padx=10, pady=10)

tk.Button(alerts_tab, text="Add Alert", command=add_alert).grid(row=6, column=0, columnspan=2, pady=20)

alert_list_frame = tk.Frame(alerts_tab)
alert_list_frame.grid(row=7, column=0, columnspan=2, pady=10)
