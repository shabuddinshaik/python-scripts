...
def save_and_start_monitoring():
    ...
    twilio_details['twiml_bin_url'] = twiml_bin_url_entry.get()
...
def save_configuration():
    config = {
        ...
        'silence_period': [silence_period[0].isoformat() if silence_period[0] else None,
                           silence_period[1].isoformat() if silence_period[1] else None]
    }
...
def load_configuration():
    global silence_period  # Ensure silence_period is treated as a global variable
    if os.path.exists('config.json'):
        with open('config.json', 'r') as config_file:
            ...
            # Handle silence_period
            silence_period_config = config.get('silence_period', (None, None))
            if isinstance(silence_period_config, list) and len(silence_period_config) == 2:
                try:
                    silence_period = (datetime.fromisoformat(silence_period_config[0]),
                                      datetime.fromisoformat(silence_period_config[1]))
                except ValueError:
                    silence_period = (None, None)
            else:
                silence_period = (None, None)
...
app.geometry('1200x900')  # Increase the window size

load_configuration()

notebook = ttk.Notebook(app)
notebook.pack(pady=10, expand=True, fill='both')  # Ensure the notebook expands to fill the window
...
auth_token_entry = ttk.Entry(twilio_frame)
auth_token_entry.insert(0, twilio_details['auth_token'])
auth_token_entry.pack()

ttk.Label(twilio_frame, text="Twilio Phone Number").pack()
...
ttk.Label(twilio_frame, text="TwiML Bin URL").pack()
twiml_bin_url_entry = ttk.Entry(twilio_frame)
twiml_bin_url_entry.insert(0, twilio_details['twiml_bin_url'])
twiml_bin_url_entry.pack()
...
# Add Save and Load Configuration Buttons in Evaluation Interval and Start/Stop Buttons Tab
...
def on_load_configuration():
    ...
    account_sid_entry.delete(0, tk.END)
    account_sid_entry.insert(0, twilio_details['account_sid'])
    auth_token_entry.delete(0, tk.END)
    auth_token_entry.insert(0, twilio_details['auth_token'])
    twilio_number_entry.delete(0, tk.END)
    twilio_number_entry.insert(0, twilio_details['twilio_number'])
    recipient_numbers_entry.delete(0, tk.END)
    recipient_numbers_entry.insert(0, ','.join(twilio_details['recipient_numbers']))
    twiml_bin_url_entry.delete(0, tk.END)
    twiml_bin_url_entry.insert(0, twilio_details['twiml_bin_url'])
    
    urls_entry.delete(0, tk.END)
    urls_entry.insert(0, ','.join(urls_to_monitor))
    http_proxy_entry.delete(0, tk.END)
    http_proxy_entry.insert(0, proxy_settings.get('http_proxy', ''))
    https_proxy_entry.delete(0, tk.END)
    https_proxy_entry.insert(0, proxy_settings.get('https_proxy', ''))
    
    mail_check_var.set(1 if mail_check_enabled else 0)
    mail_labels_entry.delete(0, tk.END)
    mail_labels_entry.insert(0, ','.join(f"{k}:{v}" for k, v in mail_labels.items()))
    
    silence_start_entry.delete(0, tk.END)
    silence_start_entry.insert(0, silence_period[0].strftime('%Y-%m-%d %H:%M:%S') if silence_period and silence_period[0] else '')
    silence_end_entry.delete(0, tk.END)
    silence_end_entry.insert(0, silence_period[1].strftime('%Y-%m-%d %H:%M:%S') if silence_period and silence_period[1] else '')

    messagebox.showinfo("Success", "Configuration loaded successfully.")

ttk.Button(eval_frame, text="Save Configuration", command=on_save_configuration).pack(pady=5)
ttk.Button(eval_frame, text="Load Configuration", command=on_load_configuration).pack(pady=5)

ttk.Button(eval_frame, text="Start Monitoring", command=save_and_start_monitoring).pack(pady=5)
ttk.Button(eval_frame, text="Stop Monitoring", command=stop_monitoring).pack(pady=5)
